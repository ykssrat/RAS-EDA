...
class GToV:
    ...
    def _parse_connections(self, conn_str: str, inst_type: str) -> List[tuple]:
        """
        解析连接字符串，返回 [(port_name, signal_name), ...]
        支持命名映射 .port(sig) 和位置映射 (sig1, sig2)
        """
        # 1. 尝试命名映射 .port(signal)
        connections = re.findall(r'\.(\w+)\s*\(([\w\d_]+)\)', conn_str)
        if connections:
            return connections
    ...
    def _generate_partition_module(self, partition_name: str) -> str:
        ...
        # 生成代码
        lines = []
        lines.append(f"// Partition {partition_name} from {self.circuit_name}")
        lines.append(f"// Contains {len(local_instances)} instances")
        lines.append("")
        
        lines.append(f"module {self.circuit_name}_{partition_name} ({', '.join(module_ports)});")
